    ...
    <div className="h-14 border-b border-[rgb(var(--border))] bg-[rgb(var(--panel))] flex items-center justify-between px-6 shrink-0 z-50 shadow-sm">
      <div className="flex items-center gap-6 h-full">
        <div className="flex items-center gap-3">
          ...
          <div className="text-[15px] font-bold tracking-wide text-[rgb(var(--text-main))] flex items-center gap-2.5 select-none">
            <div className={`w-2.5 h-2.5 rounded shadow-[0_0_10px_rgba(45,212,191,0.5)] ${fsConnected ? 'bg-[rgb(var(--accent))]' : 'bg-orange-500'}`} />
            NEXUS
          </div>
        </div>
      ...
      <div className="flex items-center gap-3">
        {!fsConnected && (
            <button
                onClick={onConnectFS}
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-[12px] font-medium bg-orange-500/10 text-orange-500 border border-orange-500/30 hover:bg-orange-500/20 transition-colors cursor-pointer"
            >
                Podłącz Folder Nexus
            </button>
        )}
