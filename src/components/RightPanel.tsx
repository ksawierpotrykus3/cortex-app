...
import { X, Lock, RefreshCw, Plus, ChevronDown, ChevronRight, Activity, Beaker, Image as ImageIcon, Loader2, MessageSquare, AlertTriangle, FileBox } from "lucide-react";
import { RightPanelState, NexusNode, NexusAnnotation } from "../types";
...
function AxiomsPanelContent({ 
  ...
  const handleAddAxiom = () => {
    setAxioms([...axioms, { id: Math.random().toString(36).substring(2, 9), text: "" }]);
  };
  ...
  return (
    <div className="flex flex-1 flex-col p-5 bg-[rgb(var(--background))] overflow-hidden">
       <div className="text-[14px] text-[rgb(var(--text-muted))] mb-6 leading-relaxed shrink-0">
           Define core axioms and rules for the system logic. These principles guide operations block evaluation.
       </div>
       
       <div className="flex-1 flex flex-col gap-4 mb-6 overflow-y-auto custom-scrollbar pr-2">
         {axioms.map((axiom, idx) => (
           <div key={axiom.id} className="flex flex-col gap-2 bg-[rgb(var(--panel))] border border-[rgb(var(--border))] rounded-2xl p-5 relative group hover:border-[rgb(var(--text-muted))]/30 transition-all shadow-sm shrink-0">
             ...
             <textarea
                ...
                className="w-full bg-transparent text-[15px] leading-relaxed text-[rgb(var(--text-main))] focus:outline-none custom-scrollbar resize-none min-h-[60px]"
             />
           </div>
         ))}
       </div>

       <div className="pt-2 shrink-0 bg-[rgb(var(--background))] border-t border-[rgb(var(--border))]/50">
...
function PropertiesPanelContent({ 
  ...
  if (!selectedNode) {
    return (
      <div className="flex flex-col p-4 items-center justify-center h-48 text-[rgb(var(--text-muted))] text-xs font-mono tracking-wider uppercase">
        NO_NODE_SELECTED
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-1 flex-col p-5 bg-[rgb(var(--background))] overflow-y-auto custom-scrollbar">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-4">
          <div>
            <label className="text-[13px] text-[rgb(var(--text-muted))] mb-1.5 block">Group</label>
            <input 
              ...
              onChange={(e) => onNodeUpdate?.(selectedNode.id, { projectId: e.target.value })}
      ...
      <div className="mt-8 pt-4 w-full shrink-0 mb-6">
        <button 
           ...
           className="w-full py-2.5 bg-[rgb(var(--background))] hover:bg-red-500/10 border border-[rgb(var(--border))] hover:border-red-500/30 text-red-500 text-[13px] font-medium transition-colors cursor-pointer rounded-lg shadow-sm"
        >
          Delete Node
        </button>
      </div>
